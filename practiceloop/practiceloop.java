...
public class practiceloop {
    public static void main(String[] args) {
        ...
        try {
            ...
            if (count > 0) {
                avg /= count; // Indicate that it cannot be divided by Zero.
            }

            System.out.println("Average = " + df.format(avg)); // Average (Indicate the df.format at the top.)
            System.out.println("Sum = " + sum); // Sum
            System.out.println("Product = " + product); // Product

            input.close(); // Input this once.
        } catch (Exception e) {
            System.out.println("Something went wrong."); // Enable the Try Catch.
        }
    }
}
